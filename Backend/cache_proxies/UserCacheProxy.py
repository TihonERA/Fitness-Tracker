...
class UserCacheProxy(BaseCacheProxy[UserInDb]):
    ...
    async def update_user(
        self, user_id: UUID, data: UserUpdate | UserUpdateDTO
    ) -> UserInDb:
        if isinstance(data, UserUpdate):
            data = UserUpdateDTO(**data.model_dump(exclude_unset=True))

        user = await self.service.update_user(user_id=user_id, data=data)

        await self.invalidator.invalidate_all(user_id)

        return self.scheme.model_validate(user)

    async def delete_user(self, user_id: UUID) -> UserInDb:
