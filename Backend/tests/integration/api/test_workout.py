...
class TestWorkoutApi:
    ...
    async def test_get_workout(self, client, workout, authorize):
        response = await client.get(f"/workouts/{workout.workout_id}")
    
        assert response.status_code == 200
        assert len(response.json()) > 0

    async def test_get_all_workouts(self, client, workout, authorize):
        response = await client.get("/workouts/get_all?my=True")

        assert response.status_code == 200
        assert len(response.json()) > 0

    async def test_update_workout(self, client, workout, authorize):
        request_body = {
            "name": "NewName",
            "description": "NewDescription"
        }
        
        response = await client.patch(f"/workouts/{workout.workout_id}", json=request_body)
        response_json = response.json()
        assert response.status_code == 200
        assert request_body.items() <= response_json.items()

       
    async def test_delete_workout(self, client, authorize, workout):
        response_delete = await client.delete(f"/workouts/{workout.workout_id}")

        assert response_delete.status_code == 200

        response_get = await client.get(f"/workouts/{workout.workout_id}")

        assert response_get.status_code == 404

    async def test_get_muscles_balance(self, client, workout):
    ...
    async def test_get_muscles_distribution_list(self, client, workout):
        workout_id = workout.workout_id
