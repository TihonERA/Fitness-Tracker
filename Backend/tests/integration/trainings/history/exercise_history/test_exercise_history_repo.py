...
class TestExerciseHistoryRepository:
    ...
    async def test_create(
        self, repo: ExerciseHistoryRepository, tr_history_data: TrDayData
    ):
        sets_history = SetsHistoryCreate(
            set=3, reps=10, weight=20.5, time_for_set=datetime.now()
        )
        ...
        created_history = await repo.create_bulk_exercise_history([data])

        assert len(created_history[0].sets_history) > 0

    async def test_get(
        self, repo: ExerciseHistoryRepository, tr_history_data: TrDayData
    ):
        fetched_history = await repo.get_exercise_history(
            tr_history_data.history.exercises_history[0].id
        )
    ...
    async def test_get_all(
        ...
        data = ExerciseHistoryGetAllDTO(
            skip=0, limit=50, user_id=tr_history_data.user_id, exercise_id=exercise_id
        )
